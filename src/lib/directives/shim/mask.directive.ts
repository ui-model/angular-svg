...
type ElementType = SVGMaskElement;

@Directive({
  selector: 'mask',
})
export class SvgMaskDirective {
  ...
  get maskUnits(): string {
    return getAttr(this.elementRef, 'maskUnits');
  }

  @Input('maskUnits')
  set maskUnits(value: string) {
    setAttr(this.elementRef, 'maskUnits', value);
  }
